...
import { useAENew } from "./useAENew";
import { styles } from "./index.styles";
import type { AENewProps } from "@app/routes/admin/stacks/AExecutors/types";

const AENew = (props: AENewProps) => {
  const { top, offset } = useMarginBottom("calc");
  const { methods, toast, isLoading, onHideToast, onSubmit } = useAENew(props);

  return (
    <ScreenContainer
      top={top}
      isSaveArea={false}>
      <HeaderUI title="Исполнитель" />
      <KeyboardContainer>
        <ScrollView
          style={styles.root}
...
export default React.memo(AENew);
